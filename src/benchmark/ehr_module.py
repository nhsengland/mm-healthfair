...
def value_embedding_data(d=512, split=200):
    vec = np.array(
        [np.arange(split) * i for i in range(d // 2)], dtype=np.float32
    ).transpose()
    vec = vec / vec.max()
...
class EHR_Embedding(nn.Module):
    def __init__(
        self, embed_type, variable_num=1, split_num=200, embed_size=256, device="cpu"
    ):
        super().__init__()
        self.device = device
        ...
        if embed_type == "demo":
            ...
            self.ethnicity = nn.Embedding(
                14, embed_size
            )  # changed to reflect num of unique ethnicity categories
        elif embed_type == "chart":
            self.var = nn.Embedding(variable_num, embed_size)
            self.value = nn.Embedding.from_pretrained(
                value_embedding_data(embed_size, split_num)
            )
            ...
            self.operation = nn.Embedding(variable_num, embed_size)
        elif embed_type == "time":
            self.value = nn.Embedding.from_pretrained(
                value_embedding_data(embed_size, split_num)
            )

    def forward(self, x):  # noqa: PLR0912, PLR0915
        if x == []:
        ...
        if self.embed_type == "demo":
            ...
            """
            demo = x.T
            anchor_age, insurance, language, marital_status, ethnicity = (
                x for x in demo
            )
            anchor_age -= 1
            marital_status += 1
            ...
            var, value = list(zip(*x))
            var, value = (
                torch.tensor(var).to(self.device),
            ...
            value[value >= 200] = 199  # noqa: PLR2004
...
class EHR_module(nn.Module):
    def __init__(self, embed_size=256, hidden_size=256, device="cpu"):
        ...
        self.chart_embed = EHR_Embedding(
            embed_type="chart", variable_num=9, embed_size=embed_size, device=device
        )
        self.lab_embed = EHR_Embedding(
            embed_type="lab", variable_num=22, embed_size=embed_size, device=device
        )
        self.procedure_embed = EHR_Embedding(
            embed_type="procedure",
            variable_num=10,
            embed_size=embed_size,
            device=device,
        )
        self.time_embed = EHR_Embedding(
            embed_type="time", embed_size=embed_size, device=device
        )
    ...
    def embedding(self, demo, chart, lab, procedure, time, pooling=False):
        ...
        embeds = []
        for idx, (chart_ts, lab_ts, procedure_ts) in enumerate(
            zip(chart, lab, procedure)
        ):
            ...
            time_embed = self.time_embed(time[idx].tolist())
        ...
        if pooling:
            embeds = list(
                map(lambda x: self.pooling(x.T.unsqueeze(0)).reshape(-1), embeds)
            )
            embeds = torch.stack(embeds)
        return embeds
